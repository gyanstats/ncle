...
with open(f'results_ncle/npe_N{N_string(num_sims)}/train_l{batch_len}.pkl', 'rb') as f:
...
def training_loop_ncl(max_epochs, x_0, num_batches):
    ...
    '''
    phi = torch.tensor([0.1], requires_grad=True) # initialise phi, enable gradient tracking
    optimizer = torch.optim.Adam([phi]) # create Adam optimiser
    ...
    for epoch in range(max_epochs):
        # Reshape phi for batch processing by duplicating its value for each of the sub-likelihoods
        context = phi.expand(batches_tensor.shape[0], 1).float()
        
        # Calculate composite log-likelihood for loss
        log_p = likelihood_estimator.log_prob(context, batches_tensor).sum()
        loss = -log_p # loss is the negative log-likelihood
...
# Calculate the score for each batch (necessary for estimating the sensitivity and variability matrices)
def calculate_scores_ncl(mcle, num_batches):
    ...
    # Stack the parameter for each observation
    mcle_stacked = mcle.unsqueeze(0).expand(num_batches, -1) # (num_batches, 1)

    # Simulate data
    x = simulator(mcle.detach(), T) # simulate x

    # Split x into batches and stack them
    batches = torch.chunk(x, num_batches)
    x_batched = torch.stack(batches) # shape: (num_batches, batch_len)

    # Use torch.func.grad with vmap over the observations dimension only
    def single_log_prob(mcle_single, x_single):
        return likelihood_estimator.log_prob(
            inputs=mcle_single.unsqueeze(0), 
            context=x_single.unsqueeze(0)
        ).squeeze()

    scores = torch.func.vmap(
        torch.func.grad(single_log_prob, argnums=0), in_dims=(0, 0)
        )(mcle_stacked, x_batched)
...
if __name__ == '__main__':
    ...
    if COVERAGE_MODE:
        ...
        filename = f'results_ncle/npe_N{N_string(num_sims)}/ci_T{N_string(T)}_l{batch_len}_task{task_id}.pkl'
    else:
        filename = f'results_ncle/npe_N{N_string(num_sims)}/ci_T{N_string(T)}_l{batch_len}.pkl'
