...
def calculate_scores_ncl(mcle, num_batches):
    ...
    '''
    # Simulate data
    x = simulator(phi=mcle.detach().item(), T=T) # simulate x
    ...
    def log_p_b(x_b, mcle_):
        ...
        converts x_b from shape (batch_len,) to (1, batch_len) and converts mcle from shape (1,) to (1, 1).
    ...
    grad_log_p_b = torch.func.grad(log_p_b, argnums=1) # differentiate log_p_b w.r.t. mcle (which is the 2nd argument)
